...
export namespace Server {
  ...
  function getErrorResponse(error: HTTPError) {
    ...
    return Response.json(
      new NamedError.Unknown({ message: getErrorMessage(cause) }).toObject(),
      { status: 500 },
    );
  }
  ...
  export const App: () => H3 = lazy(() =>
    app
      ...
      .use(async (event, next) => {
        const workspaceId =
          event.req.headers.get("x-codework-workspace") || "local";
        const raw =
          event.req.headers.get("x-codework-directory") || process.cwd();
  ...
  export async function listen(opts: {
    ...
    const startServer = async (port: number) => {
      ...
      try {
        ...
        await server.ready();
        if (!server.url)
          throw new Error(`failed to resolve server url for port: ${port}`);

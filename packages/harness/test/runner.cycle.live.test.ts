			...
			Effect.runPromise(
				Effect.gen(function* () {
					...
					const removeTerminalListener = yield* events.listen((event) => {
						...
						if (event.type === "session.llm.failed") return Queue.offer(terminals, "failed").pipe(Effect.asVoid);
						return Effect.void;
					});
					...
					expect(snapshot.messages).toHaveLength(14);
					expect(snapshot.messages[11]).toMatchObject({ role: "assistant", stopReason: "aborted" });
					expect(snapshot.messages[13]).toMatchObject({ role: "assistant", stopReason: "stop" });
					...
					expect(finalEventTypes).toHaveLength(21);
					expect(finalEventTypes.at(-1)?.type).toBe("session.llm.ended.1");

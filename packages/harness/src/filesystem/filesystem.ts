...
export class Vfs extends Context.Service<Vfs, VirtualFileSystem>()(
  "@codework/filesystem/Vfs",
) {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const readFileString = Effect.fn("FileSystem.readFileString")(function* (
      path: string,
      encoding = "utf8",
    ) {
      return yield* Effect.tryPromise({
        try: () => vfs.promises.readFile(path, encoding as BufferEncoding),
          ...
          new FileSystemError({ method: "readFileString", cause }),
      });
    });
    ...
    const writeFileString = Effect.fn("FileSystem.writeFileString")(function* (
      ...
      return yield* Effect.tryPromise({
        try: () => vfs.promises.writeFile(path, data),
          ...
          new FileSystemError({ method: "writeFileString", cause }),
      });
    });
...
export const defaultLayer = (rootPath: string) =>
  layer.pipe(Layer.provide(NodeFileSystem.layer(rootPath)));

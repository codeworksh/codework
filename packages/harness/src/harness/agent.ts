import type { Event, ThinkingBudgets } from "@codeworksh/aikit";
import { Message, Model, Protocol } from "@codeworksh/aikit";
...
export namespace Agent {
	...
	export type ToolCallInFlight = Message.ToolCallInFlight;
	...
	export class Instance {
		...
		constructor(name: string, model: Model.Info, opts: AgentOptions = {}) {
			...
			this.streamFn = opts.streamFn || Protocol.stream;
		...
		setThinkingLevel(l: Model.ThinkingLevel) {
		...
		private async _runLoop(messages?: Message.UserMessage[], opts?: { skipInitialSteeringPoll?: boolean }) {
			...
			const reasoning: Model.ActiveThinkingLevel | undefined =
				this._state.thinkingLevel === "off" ? undefined : this._state.thinkingLevel;
	...
	export async function create<TProvider extends string, TModel extends Model.Info["id"]>(

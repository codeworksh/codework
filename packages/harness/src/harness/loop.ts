import type { Event, Message } from "@codeworksh/aikit";
import { EventStream, Model, Protocol, validateToolArguments } from "@codeworksh/aikit";
...
export namespace Loop {
	...
	export type StreamFn = (
		...args: Parameters<typeof Protocol.stream>
	) => ReturnType<typeof Protocol.stream> | Promise<ReturnType<typeof Protocol.stream>>;
	...
	export interface Config extends Protocol.CommonOptions {
		model: Model.Info;
		...
		for await (const event of events) {
			switch (event.type) {
				...
				case "toolcall.end": {
					...
					endedParts.add(event.partIndex);
				...
				case "toolcall.final": {
					partialMessage = event.partial;
					await ensureMessageStarted(partialMessage);
					if (endedParts.has(event.partIndex)) {
						await emitPartUpdate(partialMessage, event.partIndex);
					} else {
						await emitPartEnd(partialMessage, event.partIndex);
						endedParts.add(event.partIndex);

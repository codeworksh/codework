...
export interface Interface {
  ...
  readonly worktreeList: (
    repo: Repo,
  ) => Effect.Effect<AbsolutePath[], WorktreeError>;
}
  ...
  Effect.gen(function* () {
    ...
    const run = (cwd: string) => (args: string[]) =>
      execute(cwd)(args).pipe(
        Effect.catch(() =>
          Effect.succeed({ exitCode: 1, text: "", stderr: "" }),
        ),
      );
    ...
    const remote = Effect.fn("Git.remote")(function* (
      repo: Repo,
      name = "origin",
    ) {
    ...
    const roots = Effect.fn("Git.roots")(function* (repo: Repo) {
      const result = yield* run(repo.directory)([
        "rev-list",
        "--max-parents=0",
        "HEAD",
      ]);
    ...
    const origin = Effect.fn("Git.origin")(function* (directory: string) {
      const result = yield* run(directory)([
        "config",
        "--get",
        "remote.origin.url",
      ]);
    ...
    const remoteHead = Effect.fn("Git.remoteHead")(function* (
      directory: string,
    ) {
      const result = yield* run(directory)([
        "symbolic-ref",
    ...
    const fetch = Effect.fn("Git.fetch")((directory: string) =>
      execute(directory)(["fetch", "--all", "--prune"]),
    );

    const fetchBranch = Effect.fn("Git.fetchBranch")(
    ...
    const worktreeList = Effect.fn("Git.worktreeList")(function* (repo: Repo) {
      return (yield* worktree("list", repo, [
        "worktree",
        "list",
        "--porcelain",
      ]))
        ...
        .map((line) =>
          AbsolutePath.make(
            resolvePath(repo.directory, line.slice("worktree ".length).trim()),
          ),
        );
    });
